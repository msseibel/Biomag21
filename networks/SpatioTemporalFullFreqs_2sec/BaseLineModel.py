...
class RunTraining():
    ...
    def test_model(self,model,test_pipe,fold=0):
        """
        ...
        if not 'readSubjects_params' in self.static_data_params.keys():
            ...
            readSubjects_params = self.static_data_params['readSubjects_params']
        with open(trainer.completeDir+r'/fold_{}'.format(fold)+r'/test_subjects.json','r') as f:
        ...
        for test_index in np.arange(len(self.data['Y'])):
            ...
            labels+=[self.meta['subjects'][test_index]['condition']]
            all_ids+=[self.meta['subjects'][test_index]['id']]
