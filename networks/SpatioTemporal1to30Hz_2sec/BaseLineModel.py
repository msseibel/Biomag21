...
class RunTraining():
    ...
    def startTrainFold(self,model,subjects_train_valid,
        ...
        # Easy Implementation, consumes more memory but allows more flexibility
            ...
            callbacks.extend([callbacks.ModelCheckpoint(self.completeDir,
                                       monitor=f1_mci_dem,
                                       save_best_only=True,
                                       mode='max')])
            train_history = model.fit(generator_train,validation_data=generator_valid, 
                ...
                max_queue_size=20)
        model.load_weights(self.completeDir)        
        return model
    ...
    def prepare_fold(self,condition_subjects_dict_train, network):
        ...
        model.compile(optimizer,loss=loss_str,metrics=self.metrics)#,options=run_opts)
    ...
    def start_main(self,network:keras.models.Model,
        ...
        with open(os.path.join(self.completeDir,'metrics.pkl'), 'wb') as output:
            pickle.dump(metrics, output, pickle.HIGHEST_PROTOCOL)  
        with open(os.path.join(self.completeDir,'callbacks.pkl'), 'wb') as output:
            pickle.dump(callbacks, output, pickle.HIGHEST_PROTOCOL)
        ...
        for k in range(n_folds):
            ...
            model, subjects_train_valid = self.prepare_fold(condition_subjects_dict_train,network)
    ...
    def continue_training(self,fold,network):
        ...
        metrics = self.load_pickle('metrics.pkl')
        callbacks = self.load_pickle('callbacks.pkl')
    ...
    def test_model(self,model,test_pipe,fold):
        """
        ...
        if not 'readSubjects_params' in self.static_data_params.keys():
            ...
            readSubjects_params = self.static_data_params['readSubjects_params']
        with open(self.completeDir+r'/fold_{}'.format(fold)+r'/test_subjects.json','r') as f:
        ...
        del self.data['X']
        return self.data['Y'],preds_soft,all_ids,labels
    
    def evaluate3classes(self,ylabels,preds_soft,fold):    
        preds = np.argmax(np.mean(preds_soft,axis=1),axis=-1)
        from libs import calc_scores
        conf_mat = calc_scores.get_confusion_matrix(ylabels,preds)
        np.savetxt(os.path.join(self.completeDir,'fold_'+str(fold),'conf_mat.txt'),conf_mat)
